...
async function eliminarVino(vinoId) {
  try {
    const response = await fetch(`https://6644bc3cb8925626f88fb766.mockapi.io/api/vins/vins/${vinoId}`, {
      method: 'DELETE'
    });
    ...
    if (!response.ok) {
      throw new Error('Error al eliminar el vino');
    }

    // Eliminar la tarjeta del vino del DOM
    document.querySelector(`[data-vino-id="${vinoId}"]`).remove();
  } catch (error) {
    console.error('Ha ocurrido un error:', error);
  }
}
...
async function generarCardsDeVinos() {
  ...
  if (vinos && usuarioActual) {
        ...
        <div class="max-w-sm rounded overflow-hidden shadow-lg m-4 md:w-1/2" data-category="${vino.categoria}" data-vino-id="${vino.id}">
          ...
          <div class="px-6 py-4 flex items-center">
            <a href="detall.html?id=${vino.id}" class="ml-auto">
              <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"><i class="fa-solid fa-book-open"></i></button>
            </a>
            ${isAdmin ? `<button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ml-4 eliminar-vino"><i class="fa-solid fa-x"></i></button>` : ''}
            ${isAdmin ? `<button class="bg-yellow-400 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded ml-4 editar-vino"><i class="fa-solid fa-pen-to-square"></i></button>` : ''}
          </div>
          <div class="like-cont">
            <svg
    ...
    document.querySelectorAll('.eliminar-vino').forEach((eliminarBtn, index) => {
      eliminarBtn.addEventListener('click', async () => {
        const vinoId = vinos[index].id;
        await eliminarVino(vinoId);
      });
    });
  } else {
